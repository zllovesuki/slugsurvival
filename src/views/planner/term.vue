...
module.exports = {
    ...
    methods: {
        ...
        exportToUCSC: function() {
            var classes = JSON.stringify(helper.compact(this.$store.getters.eventSource[this.$store.getters.termId], true));
            var script = 'javascript:(function(){function loadJquery(a){var b=document.createElement("script");b.src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.1.1/jquery.slim.min.js",b.type="text/javascript",b.onload=a,document.getElementsByTagName("head")[0].appendChild(b)}function loadBluebird(a){var b=document.createElement("script");b.src="https://cdnjs.cloudflare.com/ajax/libs/bluebird/3.4.7/bluebird.min.js",b.type="text/javascript",b.onload=a,document.getElementsByTagName("head")[0].appendChild(b)}var classes=' + classes + ',quarter="' + this.$store.getters.termName + '";loadBluebird(function(){loadJquery(function(){var a=function(){return $("#ptifrmtgtframe").contents().find("input[name=DERIVED_REGFRM1_CLASS_NBR]")[0],Promise.resolve()},b=function(a){return $($("#ptifrmtgtframe").contents().find("input[name=DERIVED_REGFRM1_CLASS_NBR]")[0]).val(a),Promise.resolve()},c=function(a){return new Promise(function(b){var c=function(){setTimeout(function(){0===$("#ptifrmtgtframe").contents().find("a").filter(function(b){return $(this).text().trim()===a}).length?c():b()},100)};c()})},d=function(){return new Promise(function(a){var b=function(){setTimeout(function(){0===$("#ptifrmtgtframe").contents().find(".PSCHECKBOX").length?b():a()},100)};b()})},e=function(a){return c("Next").then(function(){for(var c,b=$("#ptifrmtgtframe").contents().find(".PSLEVEL1GRID").find("tr"),d=1;d<b.length;d++)if(c=$(b[d]).find("td"),"null"!=a&&c[1]&&a==$(c[1]).text().trim()||c[2]&&"No Selection"==$(c[2]).text().trim()){$(c[0]).find("input")[0].click();break}})},f=function(){return d().then(function(){$("#ptifrmtgtframe").contents().find(".PSCHECKBOX")[0].click()})},g=function(){return c("Enter").then(function(){$("#ptifrmtgtframe").contents().find("a").filter(function(a){return"Enter"===$(this).text().trim()})[0].click()})},h=function(){return c("Next").then(function(){$("#ptifrmtgtframe").contents().find("a").filter(function(a){return"Next"===$(this).text().trim()})[0].click()})},i=null;return $($("#ptifrmtgtframe").contents().find(".PABOLDTEXT")[0]).text().indexOf(quarter)===-1?void alert("Wrong quarter!"):void Promise.map(classes,function(d){return i=d.split("-"),a().then(function(){return b(i[0])}).then(g).then(function(){return e(i[1]||"null")}).then(function(){return h()}).then(f).then(function(){return h()}).then(function(){return c("Enter")})},{concurrency:1})})});})()'
            var html = '';
